...
class MCPWorkflowClient:
    ...
    def analyze_dataset(self, dataset_dir: str) -> dict[str, Any]:
        ...
        result = self._call_tool(
            "dataset_analysis", {"dataset_dir": dataset_dir, "use_llm": self.use_llm}
        )

        if result.get("status") == "success":
            self.pipeline_state["normalized_metadata"] = result.get("result", {})
            self.pipeline_state["dataset_dir"] = dataset_dir
            print("[SUCCESS] Dataset analysis completed")
            print(
                f"[INFO] Extracted metadata keys: {list(self.pipeline_state['normalized_metadata'].keys())}"
            )
        else:
    ...
    def evaluate_nwb_file(self, nwb_path: Optional[str] = None) -> dict[str, Any]:
        ...
        if result.get("status") == "success":
            print("[SUCCESS] NWB evaluation completed")
    ...
    def generate_knowledge_graph(
        ...
        result = self._call_tool(
            ...
            {"nwb_path": nwb_path, "output_format": "ttl", "include_provenance": True},
        )

        if result.get("status") == "success":
            print("[SUCCESS] Knowledge graph generation completed")
    ...
    def reset_pipeline(self) -> dict[str, Any]:
        ...
        try:
            ...
            print("[SUCCESS] Pipeline state reset")
            return result
        except Exception as e:
    ...
    def run_full_pipeline(
        self, dataset_dir: str, files_map: dict[str, str], skip_kg: bool = False
    ) -> dict[str, Any]:
        """
        ...
        """
        print(f"\n{'=' * 60}")
        print("STARTING FULL NEURODATA CONVERSION PIPELINE")
        print(f"{'=' * 60}")
        ...
        print("\nPipeline Summary:")
        print(
            f"- Dataset Analysis: {'✓' if results['analysis'].get('status') == 'success' else '✗'}"
        )
        print(
            f"- Conversion: {'✓' if results['conversion'].get('status') == 'success' else '✗'}"
        )
        print(
            f"- Evaluation: {'✓' if results['evaluation'].get('status') == 'success' else '✗'}"
        )
        if not skip_kg:
            print(
                f"- Knowledge Graph: {'✓' if results.get('knowledge_graph', {}).get('status') == 'success' else '✗'}"
            )
...
def main():
    ...
    parser.add_argument(
        "--api-url", default="http://127.0.0.1:8000", help="MCP server URL"
    )
    parser.add_argument(
        "--output-dir", default="outputs", help="Output directory for results"
    )
    parser.add_argument(
        "--use-llm", action="store_true", help="Use LLM for enhanced analysis"
    )
    parser.add_argument(
        "--skip-kg", action="store_true", help="Skip knowledge graph generation"
    )
    parser.add_argument(
        "--files-map",
    ...
    client = MCPWorkflowClient(
        api_url=args.api_url, output_dir=args.output_dir, use_llm=args.use_llm
    )

    # Run pipeline
    try:
        results = client.run_full_pipeline(
            dataset_dir=args.dataset_dir, files_map=files_map, skip_kg=args.skip_kg
        )
