...
class MCPCLIWrapper:
    ...
    def analyze(
        self, dataset_dir: str, use_llm: bool = False, output_file: Optional[str] = None
    ) -> int:
        """Analyze dataset."""
    ...
    def convert(
        ...
        if metadata_file and Path(metadata_file).exists():
            ...
            analysis_result = self._call_tool(
                "dataset_analysis", {"dataset_dir": dataset_dir, "use_llm": False}
            )
    ...
    def pipeline(
        ...
        if output_dir:
            conversion_payload["output_nwb_path"] = str(
                Path(output_dir) / "converted.nwb"
            )

        conversion_result = self._call_tool(
            "conversion_orchestration", conversion_payload
        )
        ...
        evaluation_result = self._call_tool(
            "evaluate_nwb_file", {"nwb_path": nwb_path, "generate_report": True}
        )
        ...
        if not skip_kg:
            ...
            kg_result = self._call_tool(
                "generate_knowledge_graph",
                {"nwb_path": nwb_path, "output_format": "ttl"},
            )
            ...
            if kg_result.get("status") == "success":
                ...
                print(
                    f"⚠ Knowledge graph generation failed: {kg_result.get('message')}"
                )
...
Examples:
  ...
  %(prog)s pipeline /path/to/dataset --files-map '{"recording": "data.dat"}'
    ...
    analyze_parser.add_argument(
        "--use-llm", action="store_true", help="Use LLM for analysis"
    )
    ...
    convert_parser.add_argument(
        "--files-map", required=True, help="Files mapping (JSON string)"
    )
    convert_parser.add_argument("--output-nwb", help="Output NWB file path")
    convert_parser.add_argument(
        "--metadata-file", help="Use metadata from file instead of analyzing"
    )
    ...
    pipeline_parser = subparsers.add_parser(
        "pipeline", help="Run full conversion pipeline"
    )
    ...
    try:
        ...
        if args.command == "status":
            ...
            return cli.evaluate(nwb_path=args.nwb_path, output_file=args.output)
