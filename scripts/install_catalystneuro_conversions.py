...
def fetch_catalystneuro_repos():
    ...
    while True:
        try:
            ...
            url = f"https://api.github.com/orgs/catalystneuro/repos"
            params = {
                "page": page,
                "per_page": per_page,
                "type": "all"
    ...
    nwb_conversions = [
        repo['name'] for repo in all_repos 
...
def setup_datalad(base_path):
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            print("⚠️  DataLad not available in pixi environment, skipping initialization")
            return False
...
def main():
    parser = argparse.ArgumentParser(description="Install CatalystNeuro NWB conversions")
    parser.add_argument(
        "--base-path",
        type=Path,
        default=Path.cwd(),
    ...
    result = clone_conversion(COOKIECUTTER_TEMPLATE, conversions_dir, 
    ...
    for repo_name in conversions_to_install:
        result = clone_conversion(repo_name, conversions_dir, 
                                use_https=not args.use_ssh,
                                update_existing=args.update)
    ...
    print("\n📝 Next steps:")
    if stats["new"]:
        print("1. Review the new conversions in etl/input-data/catalystneuro-conversions/")
