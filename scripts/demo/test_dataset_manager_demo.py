...
def create_sample_datasets(base_dir: Path):
    ...
    (oe_dir / "metadata.json").write_text(json.dumps({
        "format": "open_ephys",
        "channels": 4,
        "sampling_rate": 30000,
    ...
    sglx_dir.mkdir()
    (sglx_dir / "demo_g0_t0.imec0.ap.bin").write_bytes(b"fake binary neural data" * 1000)
    (sglx_dir / "demo_g0_t0.imec0.ap.meta").write_text("""nSavedChans=32
sampleRate=30000.0
fileTimeSecs=60.0
imDatPrb_type=0
""")
    (sglx_dir / "metadata.json").write_text(json.dumps({
        "format": "spikeglx", 
        "channels": 32,
        "sampling_rate": 30000,
        "duration_seconds": 60
    ...
    (csv_dir / "events.csv").write_text("timestamp,event_type,description\n1.0,stimulus,visual_stimulus\n2.5,response,button_press\n")
...
def demonstrate_dataset_management():
    ...
    try:
        ...
        for dataset in all_datasets:
            print(f"   - {dataset['name']} ({dataset['dataset_type']}, {dataset['format_detected']})")
            print(f"     Files: {dataset['file_count']}, Size: {dataset['size_bytes']} bytes")
        ...
        eval_datasets = dataset_manager.get_available_datasets(dataset_type="evaluation")
        example_datasets = dataset_manager.get_available_datasets(dataset_type="example")
        ...
        print(f"   Datasets before removal: {len(dataset_manager.get_available_datasets())}")
        
        removal_result = dataset_manager.remove_dataset("demo_generic_csv", "example")
        print(f"   Removed demo_generic_csv: {removal_result}")
        
        print(f"   Datasets after removal: {len(dataset_manager.get_available_datasets())}")
