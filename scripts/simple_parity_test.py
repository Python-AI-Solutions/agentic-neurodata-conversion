...
async def test_adapter_structure():
    ...
    try:
        ...
        assert hasattr(mcp_adapter, 'conversion_service'), "MCP adapter missing conversion_service"
        assert hasattr(http_adapter, 'conversion_service'), "HTTP adapter missing conversion_service"
        print("  ✅ Both adapters have conversion_service attribute")
        
        # Check that both have initialize method
        assert hasattr(mcp_adapter, 'initialize'), "MCP adapter missing initialize method"
        assert hasattr(http_adapter, 'initialize'), "HTTP adapter missing initialize method"
...
async def test_core_service_import():
    ...
    try:
        from agentic_neurodata_conversion.core.tools import ToolSystem
...
async def main():
    ...
    print("=" * 60)
    print(f"📊 Basic Verification Summary:")
