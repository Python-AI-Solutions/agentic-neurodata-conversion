...
evaluation_agent_combined = try_import(
    ["evaluation_agent_combined", "evaluationAgentCombined", "evaluationagent_combined"]
)
...
class EvaluationAgent:
    ...
    def __init__(self, base_cache_dir: Optional[str] = None):
        ...
        if impl_cls:
            try:
                ...
                logger.exception(
                    "Failed to instantiate combined EvaluationAgent: %s", e
                )
                self._impl = None
        else:
            self._impl = None
        ...
        if self._impl is None:
            ...
            try:
                ...
                return {
                    "passed": False,
                    "counts": {"ERROR": 1},
                    "formatted_report": f"Validation error: {e}",
                    "total_issues": 1,
                }
        else:
            try:
    ...
    def generate_outputs(
        self, nwb_path: str, out_dir: Optional[str] = None, **kwargs
    ) -> dict[str, Any]:
        if self._impl is None:
            return {
                "status": "error",
                "message": "evaluation agent implementation not available",
            }
        try:
            return self._impl.generate_ttl_and_outputs(
                nwb_path, out_dir=out_dir, **kwargs
            )
        except Exception as e:
    ...
    def write_context_and_report(
        ...
        try:
            return self._impl.write_context_and_report(
                nwb_path, eval_results, outputs=outputs, out_dir=out_dir
            )
        except Exception as e:
...
async def initialize_pipeline(
    ...
    pipeline_state["workflow_history"].append(
        {
            "step": "initialize",
...
async def analyze_dataset(
    dataset_dir: str, out_report_json: Optional[str] = None, _use_llm: bool = False
) -> dict[str, Any]:
    """
    ...
    try:
        ...
        if analyze_fn is None:
            return {
                "status": "error",
                "message": "analyze_dataset function not found in conversationAgent.",
            }
        res = analyze_fn(
            dataset_dir, out_report_json=out_report_json
        )  # respects user's function signature
        pipeline_state["analysis_report"] = res
        pipeline_state["workflow_history"].append(
            {
                "step": "analyze_dataset",
                "timestamp": datetime.utcnow().isoformat(),
...
async def generate_conversion_script(
    ...
    try:
        ...
        getattr(conversionagent, "run_generated_script", None)
        if synth is None:
            return {
                "status": "error",
                "message": "synthesize_conversion_script not found in conversionagent.",
            }
        # Default output path if none
        if not output_nwb_path:
            out_dir = (
                Path(pipeline_state.get("config", {}).get("output_dir", "."))
                / "converted"
            )
            out_dir.mkdir(parents=True, exist_ok=True)
            output_nwb_path = str(
                out_dir / f"{normalized_metadata.get('dataset_name', 'converted')}.nwb"
            )
        script_text = synth(
            normalized_metadata, files_map, output_nwb_path, model=model
        )
        ...
        pipeline_state["conversion_script"] = {
            "script_path": script_path,
            "output_path": output_nwb_path,
        }
        pipeline_state["workflow_history"].append(
            {
                "step": "generate_conversion_script",
                "timestamp": datetime.utcnow().isoformat(),
                "script": script_path,
                "nwb": output_nwb_path,
            }
...
async def evaluate_nwb_file(
    ...
    if not nwb_path:
        return {
            "status": "error",
            "message": "No nwb_path provided or available from conversion_script.",
        }
    if not Path(nwb_path).exists():
        return {"status": "error", "message": f"NWB file not found at: {nwb_path}"}
    try:
        ev = EvaluationAgent(
            base_cache_dir=pipeline_state.get("config", {}).get("output_dir")
        )
        validation_results = ev.validate_nwb(nwb_path)
        outputs = None
        out_dir = output_dir or str(Path(nwb_path).parent / "evaluation_results")
        if generate_report:
            outputs = ev.generate_outputs(
                nwb_path, out_dir=out_dir, include_data=include_data
            )
            ctx = ev.write_context_and_report(
                nwb_path,
                validation_results,
                outputs=outputs.get("files") if isinstance(outputs, dict) else None,
                out_dir=out_dir,
            )
...
async def run_full_pipeline(
    ...
    try:
        ...
        analysis = await analyze_dataset(
            dataset_dir, out_report_json=None, use_llm=use_llm
        )
        ...
        if isinstance(analysis, dict) and analysis.get("status") == "success":
            normalized = analysis.get(
                "result", {}
            )  # adapt if your analyze_dataset uses different key
        # generate conversion script
        conv = await generate_conversion_script(
        ...
        if nwb_path and Path(nwb_path).exists():
            eval_res = await evaluate_nwb_file(
                nwb_path=nwb_path, generate_report=True, output_dir=output_dir
            )
        pipeline_state["workflow_history"].append(
            {"step": "run_full_pipeline", "timestamp": datetime.utcnow().isoformat()}
        )
        return {
            "status": "success",
            "analysis": analysis,
            "conversion": conv,
            "conversion_run": conv_out,
            "evaluation": eval_res,
        }
    except Exception as e:
...
async def clear_state() -> dict[str, Any]:
    ...
    pipeline_state["workflow_history"].append(
        {"step": "clear_state", "timestamp": datetime.utcnow().isoformat()}
    )
    return {"status": "ok"}
...
if __name__ == "__main__":
    ...
    if uvicorn is None or app is None:
        print(
            "FastAPI / uvicorn not available. This module defines tools and pipeline_state for import."
        )
        print(
            "To run a HTTP server install fastapi and uvicorn and run this file again."
        )
        sys.exit(0)
    print(
        f"Starting MCP server on http://{host}:{port} (tools: {list(mcp.tools.keys())})"
    )
