...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Render TTL to interactive HTML graph (PyVis)"
    )
    ap.add_argument(
        "ttl",
        nargs="?",
        default="",
        help="Path to TTL/NT/JSON-LD file. If omitted, you will be prompted.",
    )
    ap.add_argument(
    ...
    ttl_str = (args.ttl or "").strip() or input(
        "Enter path to TTL file (.ttl): "
    ).strip()
