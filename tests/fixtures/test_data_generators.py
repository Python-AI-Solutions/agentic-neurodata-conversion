...
class SyntheticDataGenerator:
    ...
    def generate_open_ephys_dataset(
        self, output_dir: Path, spec: DatasetSpec
    ) -> dict[str, Any]:
        ...
        if spec.corruption_level > 0:
            corruption_mask = (
                self.rng.random((spec.channels, samples)) < spec.corruption_level
            )
        ...
        if spec.has_events:
            ...
            with open(events_file, "w") as f:
                ...
                for event in events:
                    f.write(
                        f"{event['timestamp']},{event['type']},{event['id']},{event['description']}\n"
                    )
    ...
    def generate_spikeglx_dataset(
        ...
        if spec.corruption_level > 0:
            corruption_indices = (
                self.rng.random((samples, spec.channels)) < spec.corruption_level
            )
    ...
    def generate_generic_dataset(
        ...
        with open(data_file, "w") as f:
            ...
            for i in range(samples):
                ...
                # Add corruption if specified
                if (
                    spec.corruption_level > 0
                    and random.random() < spec.corruption_level
                ):
                    # Corrupt some values
...
class DatasetFactory:
    ...
    def create_clean_dataset(
        self, output_dir: Path, format_type: str = "open_ephys"
    ) -> dict[str, Any]:
    ...
    def create_minimal_dataset(
        self, output_dir: Path, format_type: str = "generic"
    ) -> dict[str, Any]:
    ...
    def create_large_dataset(
        self, output_dir: Path, format_type: str = "spikeglx"
    ) -> dict[str, Any]:
    ...
    def _generate_dataset(self, output_dir: Path, spec: DatasetSpec) -> dict[str, Any]:
        ...
        generator_func = generators.get(
            spec.format_type, self.generator.generate_generic_dataset
        )
