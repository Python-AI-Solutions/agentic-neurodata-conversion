...
try:
    ...
    from tests.datasets.dataset_manager import (
        TestDatasetManager,
        DatasetRegistry
    )
    ...
    from tests.datasets.access_patterns import (
        DatasetAccessor,
        DatasetCache,
        CacheEntry
    )
...
class TestDatasetRegistry:
    ...
    def test_list_datasets_with_filters(self, tmp_path):
        ...
        # Test format filter
        open_ephys_datasets = registry.list_datasets(format_filter=DatasetFormat.OPEN_EPHYS)
...
class TestDatasetFormatGenerators:
    ...
    def test_open_ephys_generator(self, tmp_path):
        ...
        result = generator.generate_dataset(
            output_dir=tmp_path,
            dataset_type=DatasetType.CLEAN,
            spec=spec
        )
    ...
    def test_generic_generator(self, tmp_path):
        ...
        result = generator.generate_dataset(
            output_dir=tmp_path,
            dataset_type=DatasetType.CORRUPTED,
            spec=spec
        )
...
class TestTestDatasetManager:
    ...
    def test_list_available_datasets(self, tmp_path):
        ...
        # Test filtering
        open_ephys_datasets = manager.list_available_datasets(format_filter=DatasetFormat.OPEN_EPHYS)
        assert len(open_ephys_datasets) == 1
