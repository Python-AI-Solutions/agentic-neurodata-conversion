...
def sample_tool_definition():
    ...
    return ToolDefinition(
        ...
        examples=[
            {"input_text": "hello", "count": 2, "uppercase": True}
        ],
        tags=["test", "utility"],
        timeout_seconds=30
    )
...
class TestToolRegistry:
    ...
    @pytest.mark.unit
    def test_tool_registration_invalid_signature(self, tool_registry, sample_tool_definition):
        """Test tool registration with invalid function signature."""
        def invalid_function(wrong_param):
            return {"result": "test"}
    ...
    @pytest.mark.unit
    def test_tool_registration_kwargs_function(self, tool_registry, sample_tool_definition):
...
class TestToolExecutor:
    ...
    @pytest.mark.unit
    async def test_execute_tool_missing_parameters(self, tool_executor, sample_tool_definition):
        ...
        with pytest.raises(ValidationError) as exc_info:
            await tool_executor.execute_tool("test_tool", {})  # Missing required input_text
        
        assert "missing required parameters" in str(exc_info.value).lower()
    
    @pytest.mark.unit
    async def test_execute_tool_invalid_parameter_type(self, tool_executor, sample_tool_definition):
    ...
    @pytest.mark.unit
    async def test_execute_tool_parameter_range_validation(self, tool_executor, sample_tool_definition):
    ...
    @pytest.mark.unit
    async def test_execute_tool_with_defaults(self, tool_executor, sample_tool_definition):
    ...
    @pytest.mark.unit
    async def test_execute_tool_exception_handling(self, tool_executor, sample_tool_definition):
    ...
    @pytest.mark.unit
    async def test_active_executions_tracking(self, tool_executor, sample_tool_definition):
...
class TestConversionToolSystem:
    ...
    async def test_tool_categories(self, conversion_tool_system):
        ...
        analysis_tools = conversion_tool_system.registry.list_tools(category=ToolCategory.ANALYSIS)
        assert len(analysis_tools) == 1
        assert analysis_tools[0].name == "dataset_analysis"
        
        conversion_tools = conversion_tool_system.registry.list_tools(category=ToolCategory.CONVERSION)
