...
"""
    ...
    async def test_analyze_directory_open_ephys(
        ...
        )  # 2 continuous + 1 events + 1 spikes + 1 settings
    ...
    @pytest.mark.unit
    async def test_extract_open_ephys_metadata(
        self, format_detector, open_ephys_dataset
    ):
        """Test Open Ephys metadata extraction."""
        metadata = await format_detector.extract_format_metadata(
            open_ephys_dataset, "open_ephys"
        )
