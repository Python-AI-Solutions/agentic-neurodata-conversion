...
try:
    from agentic_neurodata_conversion.mcp_server.server import (
        MCPServer, MCPRegistry, mcp, create_mcp_server
    )
...
class TestMCPRegistry:
    ...
    def test_tool_metadata_extraction(self, registry):
        ...
        async def test_func(param1: str, param2: int = 42, server=None):
...
class TestMCPServer:
    ...
    async def test_tool_execution_with_state_updates(self, mcp_server):
        ...
        async def state_updater_function(server=None):
            return {
                "status": "success",
