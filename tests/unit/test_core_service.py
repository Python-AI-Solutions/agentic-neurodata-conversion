...
def temp_dataset():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        (temp_path / "metadata.json").write_text('{"experiment": "test", "subject": "mouse001"}')
        (temp_path / "session_info.txt").write_text("session_id: test_session\ndate: 2024-01-01")
...
class TestConversionService:
    ...
    async def test_dataset_analysis_with_llm(self, conversion_service, temp_dataset):
        ...
        response = await conversion_service.dataset_analysis(
            dataset_dir=temp_dataset,
            use_llm=True,
            session_id="test_analysis_llm_001"
        )
    ...
    @pytest.mark.unit
    async def test_conversion_orchestration_success(self, conversion_service, sample_metadata, sample_files_map):
    ...
    @pytest.mark.unit
    async def test_conversion_orchestration_missing_metadata(self, conversion_service, sample_files_map):
    ...
    @pytest.mark.unit
    async def test_conversion_orchestration_missing_files(self, conversion_service, sample_metadata):
...
class TestErrorHandling:
    ...
    async def test_service_not_initialized_error(self):
        ...
        response = await service.dataset_analysis(
...
class TestIntegrationPoints:
    ...
    async def test_logging_integration(self, conversion_service, temp_dataset):
        ...
        with patch('agentic_neurodata_conversion.core.service.logging.getLogger') as mock_logger:
