...
class TestConversionProvenanceTracker:
    ...
    def mock_datalad(self):
        ...
        with patch(
            "agentic_neurodata_conversion.data_management.conversion_provenance.DATALAD_AVAILABLE",
            False,
        ):
            yield
    ...
    def test_tracker_initialization(self, temp_output_dir, mock_datalad):
        ...
        assert (
            tracker.conversion_repo_path
            == temp_output_dir / f"conversion_{conversion_id}"
        )
    ...
    def test_repository_structure_creation(self, temp_output_dir, mock_datalad):
        ...
        tracker = ConversionProvenanceTracker(
            conversion_id=conversion_id, output_dir=str(temp_output_dir)
        )
    ...
    def test_record_provenance(self, temp_output_dir, mock_datalad):
        ...
        tracker = ConversionProvenanceTracker(
            conversion_id="test_conversion_003", output_dir=str(temp_output_dir)
        )
        ...
        provenance_files = list(
            (tracker.conversion_repo_path / "provenance").glob("record_*.json")
        )
        assert len(provenance_files) == 2  # Init + new record

        # Check provenance log exists
        provenance_log = (
            tracker.conversion_repo_path / "provenance" / "provenance_log.jsonl"
        )
    ...
    def test_update_pipeline_state(self, temp_output_dir, mock_datalad):
        ...
        tracker = ConversionProvenanceTracker(
            conversion_id="test_conversion_004", output_dir=str(temp_output_dir)
        )
        ...
        state_update_records = [
            r
            for r in tracker.session.provenance_records
    ...
    def test_save_conversion_artifact(self, temp_output_dir, mock_datalad):
        ...
        tracker = ConversionProvenanceTracker(
            conversion_id="test_conversion_005", output_dir=str(temp_output_dir)
        )
        ...
        metadata_file = (
            tracker.conversion_repo_path / "scripts" / "test_script.py.metadata.json"
        )
    ...
    def test_finalize_conversion(self, temp_output_dir, mock_datalad):
        ...
        tracker = ConversionProvenanceTracker(
            conversion_id="test_conversion_006", output_dir=str(temp_output_dir)
        )
        ...
        session_file = (
            tracker.conversion_repo_path / "provenance" / "conversion_session.json"
        )
    ...
    def test_get_provenance_summary(self, temp_output_dir, mock_datalad):
        ...
        tracker = ConversionProvenanceTracker(
            conversion_id="test_conversion_007", output_dir=str(temp_output_dir)
        )
        ...
        assert (
            summary["confidence_stats"]["count"] == 3
        )  # Init (1.0) + user (1.0) + AI (0.7)
        assert (
            abs(summary["confidence_stats"]["average"] - 0.9) < 0.01
        )  # (1.0 + 1.0 + 0.7) / 3 ≈ 0.9
    ...
    def test_artifact_type_mapping(self, temp_output_dir, mock_datalad):
        ...
        tracker = ConversionProvenanceTracker(
            conversion_id="test_conversion_008", output_dir=str(temp_output_dir)
        )
...
class TestConversionProvenanceIntegration:
    ...
    def test_complete_conversion_workflow(self, temp_output_dir):
        ...
        tracker.update_pipeline_state({"step": "file_analysis", "files_found": 10})
...
"""
        ...
        tracker.update_pipeline_state(
            {"step": "conversion_complete", "nwb_file_created": True}
        )
