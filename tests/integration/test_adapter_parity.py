...
from agentic_neurodata_conversion.core.service import ConversionService, ConversionRequest
...
from agentic_neurodata_conversion.core.tools import ToolStatus, ToolCategory
...
def integration_dataset():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        (temp_path / "ephys_data.bin").write_bytes(b"\x00\x01\x02\x03" * 1000)  # Binary ephys data
        ...
        (temp_path / "metadata.json").write_text(json.dumps({
...
class TestCoreServiceParity:
    ...
    async def test_agent_status_parity(self, both_adapters):
        ...
        # Same agent statuses
        for agent_name in mcp_agents.keys():
    ...
    async def test_dataset_analysis_parity(self, both_adapters, integration_dataset):
        ...
        core_fields = ['analysis_type', 'metadata_extracted', 'llm_used', 'detected_formats', 'file_count']
        for field in core_fields:
            assert mcp_data.get(field) == http_data.get(field), f"Field {field} mismatch"
        
        # Execution times should be similar (within 1 second)
        time_diff = abs((mcp_response.execution_time or 0) - (http_response.execution_time or 0))
        assert time_diff < 1.0, f"Execution time difference too large: {time_diff}s"
    
    @pytest.mark.integration
    async def test_conversion_orchestration_parity(self, both_adapters, sample_metadata, sample_files_map):
    ...
    async def test_full_pipeline_parity(self, both_adapters, integration_dataset):
        ...
        # Execute pipelines
        mcp_response = await mcp_adapter.conversion_service.run_full_pipeline(mcp_request)
        http_response = await http_adapter.conversion_service.run_full_pipeline(http_request)
...
class TestSessionManagementParity:
    ...
    async def test_session_creation_parity(self, both_adapters, integration_dataset):
        ...
        mcp_sessions = list(mcp_adapter.conversion_service.workflow_orchestrator.active_sessions.keys())
        http_sessions = list(http_adapter.conversion_service.workflow_orchestrator.active_sessions.keys())
    ...
    async def test_session_status_parity(self, both_adapters, integration_dataset):
        ...
        mcp_status = await mcp_adapter.conversion_service.get_session_status(f"{session_id_base}_mcp")
        http_status = await http_adapter.conversion_service.get_session_status(f"{session_id_base}_http")
        ...
        for key in ['session_id', 'status', 'current_step', 'progress']:
            ...
            assert type(mcp_status[key]) == type(http_status[key])
...
class TestPerformanceParity:
    ...
    async def test_execution_time_parity(self, both_adapters, integration_dataset):
        ...
        if mcp_time > 0 and http_time > 0:
            ...
            assert time_ratio < 2.0, f"Execution time difference too large: MCP={mcp_time:.3f}s, HTTP={http_time:.3f}s"
...
class TestEndToEndParity:
    ...
    async def test_complete_workflow_parity(self, both_adapters, integration_dataset):
        ...
        mcp_start_response = await mcp_adapter.conversion_service.run_full_pipeline(mcp_request)
        http_start_response = await http_adapter.conversion_service.run_full_pipeline(http_request)
        ...
        if mcp_final_status["status"] == "completed":
            assert mcp_final_status.get("progress", 0) == http_final_status.get("progress", 0)
...
if __name__ == "__main__":
    ...
    pytest.main([__file__, "-v", "--tb=short"])         
       use_llm=False,
                session_id=f"{session_id_base}_mcp_{i}"
            )
            mcp_tasks.append(mcp_task)
            
            http_task = http_adapter.conversion_service.dataset_analysis(
                dataset_dir=integration_dataset,
                use_llm=False,
                session_id=f"{session_id_base}_http_{i}"
            )
        ...
        for result in mcp_results + http_results:
            assert result.status.value == "completed"
        
        # Results should be consistent across adapters
        for i in range(3):
            mcp_data = mcp_results[i].data
            http_data = http_results[i].data
            
            # Core fields should match
            assert mcp_data.get('analysis_type') == http_data.get('analysis_type')
            assert mcp_data.get('metadata_extracted') == http_data.get('metadata_extracted')
            assert mcp_data.get('llm_used') == http_data.get('llm_used')
...
class TestDataConsistencyParity:
    ...
    async def test_file_processing_parity(self, both_adapters, integration_dataset):
        ...
        summary_fields = ['total_files', 'processed_files', 'failed_files', 'skipped_files']
        for field in summary_fields:
            assert mcp_summary.get(field) == http_summary.get(field), f"Summary field {field} mismatch"
...
class TestStateManagementParity:
    ...
    async def test_session_state_parity(self, both_adapters, integration_dataset):
        ...
        # Get session states
        mcp_session = mcp_adapter.conversion_service.workflow_orchestrator.active_sessions.get(
            f"{session_id_base}_mcp"
        )
        http_session = http_adapter.conversion_service.workflow_orchestrator.active_sessions.get(
        ...
        assert type(mcp_session) == type(http_session)
    ...
    async def test_workflow_state_parity(self, both_adapters, integration_dataset):
        ...
        assert type(mcp_progress) == type(http_progress)
...
class TestUtilities:
    ...
    def compare_response_structures(response1, response2, ignore_fields=None):
        ...
        ignore_fields = ignore_fields or ['session_id', 'timestamp', 'execution_time', 'task_id']
    ...
    def extract_core_metrics(response):
        ...
        if hasattr(response, 'data') and response.data:
            return {
                ...
                'data_keys': list(response.data.keys()) if isinstance(response.data, dict) else [],
...
class TestComprehensiveParity:
    ...
    async def test_full_system_parity(self, both_adapters, integration_dataset):
        ...
        assert TestUtilities.extract_core_metrics(mcp_analysis) == TestUtilities.extract_core_metrics(http_analysis)
        ...
        mcp_pipeline = await mcp_adapter.conversion_service.run_full_pipeline(mcp_request)
        http_pipeline = await http_adapter.conversion_service.run_full_pipeline(http_request)
        ...
        mcp_status = await mcp_adapter.conversion_service.get_session_status(f"{session_id_base}_mcp_pipeline")
        http_status = await http_adapter.conversion_service.get_session_status(f"{session_id_base}_http_pipeline")
        
        if mcp_status and http_status:
            assert set(mcp_status.keys()) == set(http_status.keys())
        
        print("✅ Comprehensive parity test passed - both adapters provide identical functionality")
