...
pytestmark = pytest.mark.skipif(
    not COMPONENTS_AVAILABLE, reason="TestDatasetManager components not implemented yet"
)
...
def sample_datasets_dir(temp_repo_dir):
    ...
    (sglx_dir / "test_g0_t0.imec0.ap.meta").write_text(
        "nSavedChans=32\nsampleRate=30000\n"
    )
    ...
    csv_dir.mkdir()
    (csv_dir / "recording_data.csv").write_text(
...
class TestTestDatasetManagerIntegration:
    ...
    def test_repository_initialization_with_structure(self, temp_repo_dir):
        ...
        assert (
            dataset_manager.evaluation_data_path
            == temp_repo_dir / "etl" / "evaluation-data"
        )
        assert (
            dataset_manager.conversion_examples_path
            == temp_repo_dir / "examples" / "conversion-examples"
        )
...
class TestCompatibilityWithExistingInfrastructure:
    ...
    def test_works_with_existing_datasets_directory(self, temp_repo_dir):
        ...
        result = dataset_manager.add_test_dataset(
            "new_dataset", str(sample_dir), "New dataset alongside existing"
        )
        assert result is True
