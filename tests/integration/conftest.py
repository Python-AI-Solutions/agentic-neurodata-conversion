...
def pytest_configure(config):
    ...
    config.addinivalue_line(
        "markers", 
        "adapter_parity: Tests that verify adapter parity"
    )
    config.addinivalue_line(
        "markers",
...
def pytest_collection_modifyitems(config, items):
    ...
    for item in items:
        ...
        # Mark slow tests
        if any(keyword in item.name.lower() for keyword in ["performance", "concurrent", "comprehensive"]):
...
def mock_llm_responses():
    ...
    return {
        "dataset_analysis": {
            ...
            "recommendations": ["Convert to NWB format", "Validate metadata completeness"]
        },
        "metadata_extraction": {
