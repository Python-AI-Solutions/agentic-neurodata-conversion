...
try:
    ...
    from agentic_neurodata_conversion.mcp_server.tools import basic_tools
...
class TestMCPServerIntegration:
    ...
    async def test_basic_tool_workflow(self, mcp_server):
        ...
        state_get_result = await mcp_server.execute_tool(
            "pipeline_state",
            action="get"
        )
    ...
    def test_server_status_reporting(self, mcp_server):
        ...
        expected_fields = [
            "status", "registered_tools", "tool_names", 
            "agents", "pipeline_state_keys", "config_available"
        ]
