...
1. **Write failing tests first** - Tests should define expected behavior before implementation exists
2. **Test real interfaces** - Avoid testing mocks; test the actual components you're building
3. **Skip tests until implementation** - Use `pytest.mark.skipif` to skip tests when components don't exist yet
4. **Drive design through tests** - Let test requirements inform the design of your components
...
Tests are organized into clusters based on resource requirements and external dependencies, from lowest to highest cost:
...
This methodology ensures consistent, reliable testing across the entire project while managing resource costs and maintaining TDD principles.