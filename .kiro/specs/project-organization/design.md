...
- **Primary Interface**: Unified CLI (`agentic-converter`) with subcommands
- **Secondary Interface**: MCP server for integration with AI tools
- **Migration**: Existing scripts become internal modules, CLI provides compatibility layer

### 2. Package Structure Migration
- **Target**: `src/agentic_converter/` package structure
- **Migration**: Clean migration with import compatibility layer
- **Backward Compatibility**: 6-month deprecation period for old imports

### 3. Configuration Management
- **Primary**: `pyproject.toml` for package config, separate YAML for user config
- **Environment Variables**: Maintained for deployment, mapped to new config system
- **Migration**: Automatic detection and migration of existing env vars

### 4. DataLad Integration Scope
- **Development**: Required for project development and testing
- **Conversion Output**: Optional, enabled with `--provenance` flag
- **Fallback**: File-based tracking when DataLad not available/desired

### 5. LLM Provider Flexibility
- **Default**: OpenRouter for cloud models, Ollama for local models
- **Backward Compatibility**: Existing OpenAI env vars automatically mapped
- **Provider Interface**: Unified interface supporting multiple providers

### 6. Testing Strategy
- **LLM Testing**: Mock responses for unit tests, optional real API tests
- **DataLad Testing**: Temporary repositories, cleanup after tests
- **Integration Testing**: Docker-based testing environment

### 7. Migration Timeline
- **Phase 1**: Create new package structure alongside existing code
- **Phase 2**: Migrate core functionality, maintain compatibility layer
- **Phase 3**: Deprecate old interfaces, remove compatibility layer
