...
@mcp.tool(name="server_status", description="Get MCP server status and information")
...
@mcp.tool(
    name="list_tools", description="List all registered MCP tools with their metadata"
)
async def list_registered_tools(server=None) -> dict[str, Any]:
    ...
    try:
        ...
        return {
            "status": "success",
            "result": {"tool_count": len(tools), "tools": tools},
        }
    except Exception as e:
...
@mcp.tool(name="pipeline_state", description="Get or update pipeline state information")
async def manage_pipeline_state(
    action: str = "get", updates: dict[str, Any] = None, server=None
) -> dict[str, Any]:
    """
    ...
    try:
        if action == "get":
            ...
            return {
                "status": "success",
                "result": {"pipeline_state": state, "state_keys": list(state.keys())},
            }
            ...
            return {
                "status": "success",
                "result": {"message": "Pipeline state cleared"},
            }
...
@mcp.tool(name="echo", description="Echo back input data for testing tool execution")
async def echo_tool(
    message: str, metadata: dict[str, Any] = None, server=None
) -> dict[str, Any]:
    """
