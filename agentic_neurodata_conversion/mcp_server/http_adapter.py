...
from ..core.service import ConversionService, ConversionRequest, ConversionResponse
from ..core.tools import ConversionToolSystem, ToolStatus, ToolCategory
from ..core.exceptions import ValidationError, AgentError, ConversionError
from ..core.config import get_config, CoreConfig
...
class ConversionOrchestrationRequest(BaseModel):
    ...
    output_nwb_path: Optional[str] = Field(None, description="Output path for NWB file")
    session_id: Optional[str] = Field(None, description="Optional session ID for tracking")
...
class FullPipelineRequest(BaseModel):
    ...
    dataset_dir: str = Field(..., description="Path to dataset directory")
    files_map: Optional[Dict[str, str]] = Field(None, description="Optional file mapping")
    use_llm: bool = Field(False, description="Whether to use LLM for metadata extraction")
    output_nwb_path: Optional[str] = Field(None, description="Optional output path for NWB file")
    session_id: Optional[str] = Field(None, description="Optional session ID for tracking")
...
class HTTPAdapter:
    ...
    def __init__(self, 
        ...
        self.tool_system: Optional[ConversionToolSystem] = None
    ...
    def _register_routes(self) -> None:
        ...
        async def health_check():
            ...
            return HealthCheckResponse(
                ...
                tools=len(self.tool_system.registry.list_tools()) if self.tool_system else 0,
                active_sessions=len(self.conversion_service.workflow_orchestrator.active_sessions)
            )
        ...
        async def list_tools(category: Optional[str] = None):
            ...
            for tool in tools:
                ...
                for param in tool.parameters:
                    http_params.append(HTTPToolParameter(
                        name=param.name,
                ...
                result.append(HTTPToolDefinition(
                    name=tool.name,
        ...
        async def run_full_pipeline(request: FullPipelineRequest):
            ...
            response = await self.conversion_service.run_full_pipeline(conversion_request)
        ...
        async def list_sessions():
            ...
            return list(self.conversion_service.workflow_orchestrator.active_sessions.keys())
        
        @self.app.get("/sessions/{session_id}", response_model=HTTPSessionStatus)
        async def get_session_status(session_id: str):
            """Get status of a specific conversion session."""
            session_status = await self.conversion_service.get_session_status(session_id)
            ...
            return HTTPSessionStatus(
                ...
                end_time=datetime.fromisoformat(session_status["end_time"]) if session_status["end_time"] else None,
        ...
        async def cancel_session(session_id: str):
            ...
            if not success:
                raise HTTPException(status_code=404, detail=f"Session not found or cannot be cancelled: {session_id}")
            
            return {"message": f"Session {session_id} cancelled successfully"}
    
    async def run_server(self, host: str = "127.0.0.1", port: int = 8000, **kwargs) -> None:
        """Run the HTTP server.
...
async def main():
    ...
    try:
        await adapter.run_server(
            ...
            reload=settings.mcp_server.debug
        )
