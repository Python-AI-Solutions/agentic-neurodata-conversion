...
class TestDatasetManager:
    ...
    def __init__(self, repository_manager: DataLadRepositoryManager):
        ...
        self.evaluation_data_path = repository_manager.base_path / 'etl' / 'evaluation-data'
        self.conversion_examples_path = repository_manager.base_path / 'examples' / 'conversion-examples'
...
"""
    ...
    def install_subdataset(self, subdataset_url: str, subdataset_path: str, 
    ...
    def get_available_datasets(self, dataset_type: Optional[str] = None) -> List[Dict[str, Any]]:
        """
    ...
    def get_dataset_path(self, dataset_name: str, dataset_type: str = "test") -> Optional[Path]:
        """
    ...
    def remove_dataset(self, dataset_name: str, dataset_type: str = "test") -> bool:
        ...
        try:
            ...
            if DATALAD_AVAILABLE and (self.repo_manager.base_path / '.datalad').exists():
                try:
                    dataset = Dataset(self.repo_manager.base_path)
                    dataset.remove(path=str(dataset_path), 
    ...
    def list_subdatasets(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for metadata_file in self.repo_manager.base_path.rglob('subdataset_metadata.json'):
                try:
                    metadata = json.loads(metadata_file.read_text())
                    metadata['metadata_path'] = str(metadata_file)
                    subdatasets.append(metadata)
                except Exception as e:
                    self.logger.warning(f"Could not read subdataset metadata {metadata_file}: {e}")
    ...
    def _get_dataset_info(self, dataset_path: Path, dataset_type: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            metadata['relative_path'] = str(dataset_path.relative_to(self.repo_manager.base_path))
            ...
            if DATALAD_AVAILABLE and (self.repo_manager.base_path / '.datalad').exists():
                try:
                    ...
                    self.logger.debug(f"Could not get DataLad status for {dataset_path}: {e}")
