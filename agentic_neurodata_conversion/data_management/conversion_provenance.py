...
class ConversionProvenanceTracker:
    ...
    def __init__(self, conversion_id: str, output_dir: str, dataset_path: str = ""):
        ...
        if not DATALAD_AVAILABLE:
            self.logger.warning("DataLad not available. Provenance tracking will be limited.")
        ...
        self.record_provenance(
            ...
            output_data={"session_id": conversion_id, "output_path": str(self.conversion_repo_path)},
            confidence=1.0,
        ...
        try:
            ...
            if DATALAD_AVAILABLE:
                try:
                    ...
                    self.logger.info(f"Created DataLad repository: {self.conversion_repo_path}")
...
'''
    ...
    def _save_provenance_record(self, record: ProvenanceRecord):
        ...
        record_data['timestamp_human'] = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(record.timestamp))
    ...
    def update_pipeline_state(self, state_updates: Dict[str, Any]):
        ...
        state_data = {
            ...
            'update_count': len([r for r in self.session.provenance_records if r.operation == 'state_update'])
        }
    ...
    def finalize_conversion(self, status: str, final_metadata: Dict[str, Any]):
        ...
        for record in session_data['provenance_records']:
            if isinstance(record['source'], ProvenanceSource):
                record['source'] = record['source'].value
            elif hasattr(record['source'], 'value'):
                record['source'] = record['source'].value
        ...
        if self.dataset:
            try:
                self.dataset.save(
                    ...
                    path=[str(session_file), str(summary_file), str(self.conversion_repo_path / 'README.md')]
                )
...
"""
        ...
        confidence_records = [r for r in self.session.provenance_records if r.confidence is not None]
        if confidence_records:
            avg_confidence = sum(r.confidence for r in confidence_records) / len(confidence_records)
            high_confidence = len([r for r in confidence_records if r.confidence >= 0.8])
    ...
    def get_provenance_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'confidence_stats': {
                ...
                'average': sum(confidence_stats) / len(confidence_stats) if confidence_stats else None,
