...
class FormatDetector:
    ...
    def __init__(self):
        ...
        logger.info(
            f"Format detector initialized with {len(self._format_patterns)} known formats"
        )
    ...
    def detect_format(
        ...
        if data_path.is_file():
            ...
            detection_result["detected_formats"] = detection_result[
                "file_analysis"
            ].get("formats", [])
        else:
            detection_result["directory_analysis"] = self._analyze_directory(
                data_path, recursive
            )
            detection_result["detected_formats"] = detection_result[
                "directory_analysis"
        ...
        if detection_result["detected_formats"]:
            confidences = [
                fmt.get("confidence", 0.0)
                for fmt in detection_result["detected_formats"]
            ]
            detection_result["confidence"] = max(confidences)

        logger.info(
            f"Format detection completed. Found {len(detection_result['detected_formats'])} potential formats"
        )
    ...
    def _analyze_directory(self, directory: Path, recursive: bool) -> dict[str, Any]:
        ...
        directory_analysis["extensions_found"] = list(
            directory_analysis["extensions_found"]
        )
        ...
        for format_name, format_info in self._format_patterns.items():
            ...
            for required_pattern in format_info["required_files"]:
                ...
                if recursive:
                    matching_files.extend(
                        list(directory.glob(f"**/{required_pattern}"))
                    )
            ...
            if format_info["required_files"] and required_matches > 0:
                confidence += 0.6 * (
                    required_matches / len(format_info["required_files"])
                )
            ...
            for optional_pattern in format_info["optional_files"]:
                ...
                if recursive:
                    matching_files.extend(
                        list(directory.glob(f"**/{optional_pattern}"))
                    )
            ...
            if format_info["optional_files"] and optional_matches > 0:
                confidence += 0.2 * (
                    optional_matches / len(format_info["optional_files"])
                )
            ...
            if format_info["extensions"] and extension_matches > 0:
                ...
                matches.append(
                    f"Extensions found: {extension_matches}/{len(format_info['extensions'])}"
                )
