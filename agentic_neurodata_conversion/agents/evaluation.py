...
class EvaluationAgent(BaseAgent):
    ...
    def _initialize(self) -> None:
        ...
        logger.info(f"EvaluationAgent {self.agent_id} initialized with strict_mode={self.strict_mode}")
    ...
    def _initialize_validation_rules(self) -> None:
        ...
        self.validation_rules = {
            "file_structure": {
                ...
                "required_datasets": ["/general/session_description", "/general/experimenter"],
    ...
    async def process(self, task: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if task_type == "validation":
            ...
            raise NotImplementedError(f"Task type '{task_type}' not supported by EvaluationAgent")
    ...
    async def _validate_nwb_file(self, task: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            # Perform validation
            validation_result = await self._perform_nwb_validation(nwb_file, validation_level)
    ...
    async def _perform_nwb_validation(self, nwb_file: Path, validation_level: str) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            logger.info(f"NWB validation completed for {nwb_file}: {validation_result['overall_status']}")
            
        except Exception as e:
            validation_result["overall_status"] = "error"
            validation_result["issues"]["critical"].append(f"Validation failed: {str(e)}")
    ...
    async def _validate_file_structure(self, nwb_file: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            required_datasets = self.validation_rules["file_structure"]["required_datasets"]
    ...
    async def _validate_metadata_completeness(self, nwb_file: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            required_fields = self.validation_rules["metadata_completeness"]["required_fields"]
            recommended_fields = self.validation_rules["metadata_completeness"]["recommended_fields"]
            ...
            metadata_result["completeness_score"] = present_required / total_required if total_required > 0 else 1.0
            ...
            logger.debug(f"Metadata validation completed for {nwb_file}: {metadata_result['completeness_score']:.2f}")
    ...
    async def _validate_data_integrity(self, nwb_file: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            integrity_result["unit_checks"]["issues"].append("Some units may be missing or inconsistent")
            integrity_result["reference_checks"]["status"] = "passed"
            
            # Overall status based on individual checks
            if any(check["status"] == "error" for check in integrity_result.values() if isinstance(check, dict)):
                integrity_result["status"] = "error"
            elif any(check["status"] == "warning" for check in integrity_result.values() if isinstance(check, dict)):
    ...
    async def _run_nwb_inspector(self, nwb_file: Path, validation_level: str) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if validation_level == "error":
                inspector_result["messages"] = [
                    ...
                    {"level": "warning", "message": "Some optional metadata is missing"}
                ]
            ...
            inspector_result["status"] = "passed" if inspector_result["error_count"] == 0 else "failed"
    ...
    def _aggregate_validation_results(self, validation_result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        validation_result["summary"] = {
            ...
            "failed_checks": sum(1 for check in validation_result["checks"].values() 
                               if isinstance(check, dict) and check.get("status") == "error"),
            "warning_checks": sum(1 for check in validation_result["checks"].values() 
    ...
    async def _evaluate_nwb_file(self, task: Dict[str, Any]) -> Dict[str, Any]:
        ...
        evaluation_result["evaluation"] = {
            ...
            "conversion_success": evaluation_result["overall_status"] in ["passed", "warning"]
        }
    ...
    def _calculate_quality_score(self, validation_result: Dict[str, Any]) -> float:
        ...
        warning_penalty = (warning_checks / total_checks) * 0.2 if total_checks > 0 else 0.0
    ...
    def _generate_recommendations(self, validation_result: Dict[str, Any]) -> List[str]:
        ...
        metadata_check = validation_result.get("checks", {}).get("metadata_completeness", {})
        ...
        if missing_required:
            recommendations.append(f"Add missing required metadata fields: {', '.join(missing_required)}")
        
        if missing_recommended:
            recommendations.append(f"Consider adding recommended metadata fields: {', '.join(missing_recommended[:3])}")
        
        # Check data integrity
        integrity_check = validation_result.get("checks", {}).get("data_integrity", {})
        if integrity_check.get("status") == "warning":
            recommendations.append("Review data integrity warnings and ensure data consistency")
        
        # General recommendations
        if validation_result.get("overall_status") == "warning":
            recommendations.append("Address validation warnings to improve NWB file quality")
    ...
    def _create_report_content(self, evaluation_result: Dict[str, Any], report_format: str) -> str:
        """
