...
class BaseAgent(ABC):
    ...
    def __init__(self, config: Optional[Any] = None, agent_id: Optional[str] = None):
        ...
        # Event handlers
        self._status_change_handlers: List[Callable[[AgentStatus, AgentStatus], None]] = []
        self._error_handlers: List[Callable[[Exception], None]] = []
        
        logger.info(f"Initializing agent {self.agent_id} of type {self.__class__.__name__}")
    ...
    def can_handle_task(self, task: Dict[str, Any]) -> bool:
        ...
        return required_capability is not None and self.has_capability(required_capability)
    ...
    async def execute_task(self, task: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if self.status != AgentStatus.READY:
            raise RuntimeError(f"Agent {self.agent_id} is not ready (status: {self.status.value})")
    ...
    def add_status_change_handler(self, handler: Callable[[AgentStatus, AgentStatus], None]) -> None:
        """
    ...
    def _notify_status_change(self, old_status: AgentStatus, new_status: AgentStatus) -> None:
        """
...
class AgentRegistry:
    ...
    def unregister_agent(self, agent_id: str) -> Optional[BaseAgent]:
        ...
        for capability in agent.get_capabilities():
            if capability in self.agents_by_capability:
                self.agents_by_capability[capability] = [
                    a for a in self.agents_by_capability[capability] if a.agent_id != agent_id
                ]
    ...
    def find_agent_for_task(self, task: Dict[str, Any]) -> Optional[BaseAgent]:
        ...
        return min(suitable_agents, key=lambda a: a.error_count / max(a.success_count + a.error_count, 1))
    ...
    def get_registry_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "agents_by_type": {
                agent_type: len(agents) for agent_type, agents in self.agents_by_type.items()
            },
