...
class ConversionAgent(BaseAgent):
    ...
    def _initialize(self) -> None:
        ...
        logger.info(
            f"ConversionAgent {self.agent_id} initialized with {len(self.script_templates)} templates"
        )
...
'''
    ...
    def get_capabilities(self) -> set[AgentCapability]:
        ...
        return {AgentCapability.SCRIPT_GENERATION, AgentCapability.CONVERSION_EXECUTION}
    ...
    async def process(self, task: dict[str, Any]) -> dict[str, Any]:
        ...
        if task_type == "script_generation":
            ...
            raise NotImplementedError(
                f"Task type '{task_type}' not supported by ConversionAgent"
            )
    ...
    async def _generate_script(self, task: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            return {"status": "success", "result": result, "agent_id": self.agent_id}
    ...
    def _create_script_file(self, script_content: str) -> Path:
        ...
        with tempfile.NamedTemporaryFile(
            mode="w", suffix=".py", prefix="neuroconv_script_", delete=False
        ) as f:
    ...
    async def _execute_conversion(self, task: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            self.active_conversions[execution_id]["end_time"] = (
    ...
    async def _run_conversion_script(
        ...
        try:
            ...
            if process.returncode != 0:
                raise RuntimeError(
                    f"Conversion script failed with code {process.returncode}: {stderr.decode()}"
                )
            ...
            raise RuntimeError(
                f"Conversion script timed out after {timeout} seconds"
            ) from None
