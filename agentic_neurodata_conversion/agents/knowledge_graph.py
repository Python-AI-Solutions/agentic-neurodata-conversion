...
class KnowledgeGraphAgent(BaseAgent):
    ...
    def _initialize(self) -> None:
        ...
        logger.info(
            f"KnowledgeGraphAgent {self.agent_id} initialized with format={self.output_format}"
        )
    ...
    def _initialize_ontologies(self) -> None:
        ...
        self.ontologies = {
            ...
            "provenance": {
                ...
                "concepts": ["Activity", "Entity", "Agent", "Generation", "Usage"],
            },
        }
    ...
    async def process(self, task: dict[str, Any]) -> dict[str, Any]:
        ...
        if task_type == "knowledge_graph":
            ...
            raise NotImplementedError(
                f"Task type '{task_type}' not supported by KnowledgeGraphAgent"
            )
    ...
    async def _build_knowledge_graph(
        ...
        try:
            ...
            logger.info(
                f"Built knowledge graph {graph_id} with {len(entities)} entities and {len(relationships)} relationships"
            )
    ...
    async def _extract_nwb_entities(
        self, nwb_file: Path, metadata: dict[str, Any]
    ) -> dict[str, Any]:
        """
    ...
    async def _build_provenance_graph(
        self, nwb_file: Path, metadata: dict[str, Any]
    ) -> dict[str, Any]:
        """
    ...
    def _calculate_graph_statistics(
        ...
        statistics = {
            ...
            "provenance_activities": len(provenance.get("activities", {}))
            if provenance
            else 0,
        ...
        try:
            ...
            if query_type == "entity_search":
                ...
                results = self._find_paths(
                    knowledge_graph, task.get("start_entity"), task.get("end_entity")
                )
            else:
    ...
    def _search_entities(
        ...
        for entity_id, entity in entities.items():
            ...
            if (
                query_lower in entity_id.lower()
                or query_lower in entity.get("type", "").lower()
                or any(
                    query_lower in str(value).lower()
                    for value in entity.get("properties", {}).values()
                )
            ):
                results.append(
                    {
    ...
    def _search_relationships(
        ...
        for rel in relationships:
            ...
            if (
                query_lower in rel.get("predicate", "").lower()
                or query_lower in rel.get("type", "").lower()
            ):
                results.append({"relationship": rel, "match_score": 1.0})

        return results

    def _find_paths(
        self, knowledge_graph: dict[str, Any], start_entity: str, end_entity: str
    ) -> list[list[str]]:
    ...
    def _generate_export_content(
        self, knowledge_graph: dict[str, Any], export_format: str
    ) -> str:
        """
    ...
    def _generate_jsonld_content(self, knowledge_graph: dict[str, Any]) -> str:
        ...
        jsonld = {"@context": knowledge_graph.get("namespaces", {}), "@graph": []}

        # Add entities
        entities = knowledge_graph.get("entities", {})
        for entity_id, entity in entities.items():
            jsonld_entity = {"@id": entity_id, "@type": entity.get("type")}
    ...
    async def _merge_knowledge_graphs(self, task: dict[str, Any]) -> dict[str, Any]:
        ...
        merged_graph_id = task.get(
            "merged_graph_id", f"merged_{len(self.knowledge_graphs)}"
        )
    ...
    def remove_knowledge_graph(self, graph_id: str) -> bool:
        ...
        if graph_id in self.knowledge_graphs:
            ...
            cache_keys_to_remove = [key for key in self.graph_cache if graph_id in key]
