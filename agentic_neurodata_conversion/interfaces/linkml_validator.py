...
class LinkMLValidator:
    ...
    def validate_nwb_metadata(
        ...
        """
        logger.info(f"Validating NWB metadata against schema version: {nwb_schema_version}")
        
        # For now, use generic validation with NWB-specific checks
        validation_result = self.validate_data(
            metadata,
            f"nwb_schema_{nwb_schema_version}",
        ...
        if validation_result["errors"]:
            validation_result["status"] = "invalid"
            validation_result["summary"]["error_count"] = len(validation_result["errors"])
