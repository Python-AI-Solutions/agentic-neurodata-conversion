...
from agentic_neurodata_conversion.models.mcp import MCPEvent, MCPMessage, MCPResponse
from agentic_neurodata_conversion.models.state import GlobalState, LogLevel
...
class MCPServer:
    ...
    def __init__(self):
        ...
        self._agent_instances: dict[str, Any] = {}  # Optional agent instance storage (primarily for testing)
    ...
    def agents(self, value: dict[str, Any]) -> None:
        """Set registered agent instances.

        Args:
            value: Dictionary mapping agent names to their instances
        """
        self._agent_instances = value

    @property
    def handlers(self) -> dict[str, dict[str, Callable]]:
        """Get registered handler functions.

        Returns:
            Dictionary mapping agent names to their action handlers
        """
    ...
    def register_handler(
        ...
        agent_name: str,
        action: str,
        handler: Callable[[MCPMessage, GlobalState], Coroutine[Any, Any, MCPResponse]],
    ) -> None:
        ...
        Args:
            ...
            handler: Async callable that processes the message and returns MCPResponse coroutine
        """
    ...
    async def send_message(self, message: MCPMessage) -> MCPResponse:
        ...
        try:
            response: MCPResponse = await handler(message, self._global_state)
            self._global_state.add_log(
                LogLevel.DEBUG,
