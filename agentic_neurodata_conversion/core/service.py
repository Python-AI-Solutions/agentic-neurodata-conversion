...
class ConversionRequest(BaseModel):
    ...
    files_map: Optional[Dict[str, str]] = Field(
        None, 
        description="Optional mapping of file types to specific paths"
    )
...
class SessionState(BaseModel):
    ...
    dataset_dir: Optional[str] = Field(None)
    files_map: Optional[Dict[str, str]] = Field(None)
    use_llm: bool = Field(False)
    output_nwb_path: Optional[str] = Field(None)
...
class AgentManager:
    ...
    async def _execute_agent_operation(self, agent_type: str, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            if nwb_path:
                ...
                if evaluation_result.status == ConversionStatus.FAILED:
                    ...
                    session.warnings.append(f"Evaluation failed: {evaluation_result.error}")
                else:
