...
class ToolParameter(BaseModel):
    ...
    def validate_name(cls, v: str) -> str:
        ...
        if not v.replace("_", "").replace("-", "").isalnum():
            raise ValueError("Parameter name must be alphanumeric with underscores or hyphens")
        return v
...
class ToolMetrics(BaseModel):
    ...
    min_execution_time: Optional[float] = Field(None)
    max_execution_time: Optional[float] = Field(None)
...
class ToolRegistry:
    ...
    def register_tool(
        ...
        self.logger.info(f"Registered tool: {definition.name} ({definition.category.value})")
    ...
    def update_metrics(self, execution: ToolExecution) -> None:
        ...
        if execution.execution_time is not None:
            if metrics.min_execution_time is None or execution.execution_time < metrics.min_execution_time:
                metrics.min_execution_time = execution.execution_time
            
            if metrics.max_execution_time is None or execution.execution_time > metrics.max_execution_time:
            ...
            metrics.average_execution_time = (total_time + execution.execution_time) / metrics.total_executions
...
class ToolExecutor:
    ...
    async def execute_tool(
        ...
        try:
            ...
            execution.end_time = datetime.now(timezone.utc)
            execution.error = f"Tool execution timed out after {tool_def.timeout_seconds} seconds"
    ...
    def _validate_parameter_value(
        ...
        elif param_def.type == ParameterType.NUMBER and not isinstance(value, (int, float)):
            raise ValidationError(
...
class ConversionToolSystem:
    ...
    def _register_conversion_tools(self) -> None:
        ...
        self.registry.register_tool(
            dataset_analysis_def,
            self._dataset_analysis_wrapper
        )
        ...
        self.registry.register_tool(
            conversion_def,
            self._conversion_orchestration_wrapper
        )
        ...
        self.registry.register_tool(
            pipeline_def,
            self._run_full_pipeline_wrapper
        )
