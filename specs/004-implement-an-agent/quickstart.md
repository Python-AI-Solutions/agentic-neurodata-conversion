...
## Overview
This guide provides a step-by-step test scenario to validate the agent-based neurodata conversion system. It covers the primary user story: a researcher using natural language to request conversion, the agent gathering requirements, executing the workflow, and reporting results.
...
**Objective**: Initiate a conversation with the agent to describe conversion needs.
...
**Objective**: Answer agent's clarifying questions with specific file information.
...
**Objective**: Get comprehensive task information including decisions made by the agent.
...
**Objective**: Confirm the converted NWB file is readable and contains expected data.
...
Test that status queries return accurate progress information while conversion is running.
...
## Success Criteria
All validation checkboxes above must be checked for the quickstart to pass. This ensures:
